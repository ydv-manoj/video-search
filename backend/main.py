...
def get_embedding_with_gemini(text: str) -> Optional[List[float]]:
    ...
    try:
        response = genai_client.models.embed_content(
            ...
            config=EmbedContentConfig(
                ...
                title="Video Transcript Segment",
            ),
        )
...
def store_in_chromadb(video_id: str, video_name: str, chunks: List[dict]):
    """Store transcription chunks in ChromaDB with Gemini embeddings"""
    documents = []
    metadatas = []
    ids = []
    embeddings = []
    ...
    for i, chunk in enumerate(chunks):
        if chunk["text"].strip():  # Only store non-empty chunks
            ...
            embedding = get_embedding_with_gemini(text)
...
async def search_videos(
    ...
    try:
        ...
        query_embedding = get_embedding_with_gemini(query)
